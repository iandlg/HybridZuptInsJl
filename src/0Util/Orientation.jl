...
function jacobian_∂θ3_∂δθ(R::AbstractMatrix{T})::AbstractVector{T} where T<:Real
    size(R) == (3, 3) || throw(DimensionMismatch("Expected matrix of size (3,3), got $(size(R))"))
    denom = (R[1, 1]^2 + R[2, 2]^2)
    denom < 1e-9 && @warn "`jacobian_∂θ3_∂δθ` Close to singular division, denom = $(denom)"
    return [
        0,
...
function jacobian_∂θ3_∂δθ(q::AbstractVector{T})::AbstractVector{T} where T<:Real
    length(q) == 4 || throw(DimensionMismatch("Expected vector of length 4, got $(length(q))"))
    return jacobian_∂θ3_∂δθ(quat_to_matrix(q))
end
