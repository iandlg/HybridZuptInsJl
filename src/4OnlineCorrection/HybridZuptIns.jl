...
- `true_outputs`:       Dictionary with ground-truth error observations ("yaw" and "pos").
- `predicted_outputs`:  Dictionary with GP predictions ("yaw" and "pos").
"""
function hybrid_nominal_zupt_aided_ins(
    ...
    predicted_outputs = Dict{String,Any}(
        "yaw" => Float64[],
        "pos" => Vector{Float64}[]
    )
    ...
    while true
        ...
        if length(step_seg) > 1
            ...
            input_feature = reshape(
                (ins_step .- gp_params.input_stats[1]) ./ gp_params.input_stats[2],
            ...
            if gt_available[prev_step] && gt_available[curr_step]
                ...
                # Normilize training outputs
                y[1] = (y[1] - gp_params.yaw_stats[1]) / gp_params.yaw_stats[2]
                y[2:4] = (y[2:4] .- gp_params.pos_stats[1]) ./ gp_params.pos_stats[2]
                ...
                # Save unnormalized training data
            ...
            if !gt_available[curr_step] && correction
                ...
                preds[1] = preds[1] * gp_params.yaw_stats[2] + gp_params.yaw_stats[1]
                preds[2:4] = preds[2:4] .* gp_params.pos_stats[2] + gp_params.pos_stats[1]

                # Save predicted outputs
                push!(predicted_outputs["yaw"], preds[1])
                push!(predicted_outputs["pos"], preds[2:4])
    ...
    return zupt, zupt_ins_traj, step_seg, true_outputs, predicted_outputs
end
...
function hybrid_nominal_zupt_aided_ins(
    ...
    true_outputs = Dict{String,Any}("yaw" => Float64[], "pos" => Vector{Float64}[])
    pred_outputs = Dict{String,Any}("yaw" => Float64[], "pos" => Vector{Float64}[])
    ...
    while true
        ...
        if length(step_seg) > 1
            ...
            if gt_available[prev_step] && gt_available[curr_step]
                ...
                x[1:3, curr_step] = gt_traj.pos[:, curr_step]
                quat[:, curr_step] = matrix_to_quat(gt_traj.R_nb[:, :, curr_step])
                # x[7:9, curr_step] = matrix_to_euler(gt_traj.R_nb[:, :, curr_step])
            end
